...
from dots import split_field
from parsing_future import first, is_text, string_types, text
from parsing import listwrap

from sql_parsing.keywords import RESERVED, join_keywords, precedence
from sql_parsing.utils import binary_ops, is_set_op
...
class Formatter:
    ...
    def _literal(self, json, prec=0):
        if isinstance(json, list):
            ...
            return self._literal(json["literal"])
        else:
            return str(json)
    ...
    def insert(self, json, prec=precedence["from"]):
        ...
        if "values" in json:
            ...
            query_values = json["query"]
            if "select" in query_values:
                values = query_values["select"]
                if all(isinstance(row, dict) for row in values):
                    if "if exists" in json:
                        acc.append("IF EXISTS")
                    acc.append("VALUES")
                    acc.append(
                        "(" + ", ".join(self._literal(v['value']) for v in values) + ")"
                    )
