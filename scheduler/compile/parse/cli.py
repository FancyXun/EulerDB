...
Why does this file exist, and why not put this in __main__?
  ...
  - When you run `python -m parse` python will execute
    ``__main__.py`` as a script. That means there won't be any
    ``parse.__main__`` in ``sys.modules``.
  - When you import __main__ it will get executed again (as a module) because
    there's no ``parse.__main__`` in ``sys.modules``.
...
from scheduler.compile.parse.exceptions import SQLParseError
...
def main(args=None):
    ...
    try:
        formatter_opts = parser.formatter.validate_options(formatter_opts)
    except SQLParseError as e:
        return _error('Invalid options: {}'.format(e))

    s = parser.format(data, **formatter_opts)
    stream.write(s)
    stream.flush()
    if close_stream:
        stream.close()
