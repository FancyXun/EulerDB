...
from scheduler.lex_module.lacquer.tree import AstVisitor, TableSubquery, JoinUsing, JoinOn, \
    NaturalJoin, ExplainType, ExplainFormat, Table, SimpleGroupBy

from scheduler.lex_module.lacquer.utils import FIELD_REFERENCE_PREFIX
...
class Formatter(AstVisitor):
    ...
    # def visit_binary_literal(self, node, Boolean unmangle_names):
    #     return "X'{}'".format(binascii.hexlify(node))

    # def visit_array_constructor(self, node, unmangle_names):
    #     value_strings = [format_sql(val, unmangle_names) for val in node.values]
    #         value_strings.add(format_sql(value, unmangle_names))
    ...
    # def visit_generic_literal(self, node, unmangle_names):
    ...
    # def visit_lambda_expression(self, node, unmangle_names):
    #     pass
