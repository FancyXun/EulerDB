...
from ply.sql_metadata.generalizator import Generalizator
from ply.sql_metadata.keywords_lists import (
    COLUMNS_SECTIONS,
...
from ply.sql_metadata.token import EmptyToken, SQLToken
from ply.sql_metadata.utils import UniqueList, flatten_list
...
class Parser:  # pylint: disable=R0902
    ...
    @property
    def columns_type(self) -> List[str]:
        ...
        for token in self._not_parsed_tokens:
            if token.is_name or token.is_keyword_column_name:
                if token.is_column_definition_inside_create_table(
                    ...
                    columns_type.append(token.next_token.value)
                elif (
