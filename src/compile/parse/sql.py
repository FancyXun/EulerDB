...
from src.compile.parse import tokens as T
from src.compile.parse.utils import imt, remove_quotes
...
class Token:
    ...
    def within(self, group_cls):
        ...
        a function: ``t.within(compile.Function)``.
        """
        parent = self.parent
        while parent:
...
class TokenList(Token):
    ...
    def encrypt_tokens(self, grp, start, end, include_end=True):
        ...
        for idx, subtoken in enumerate(subtokens):
            if isinstance(subtoken, grp):
                tokens = subtoken.tokens[1].tokens
...
class IdentifierList(TokenList):
    """A list of :class:`~parse.compile.Identifier`\'s."""
