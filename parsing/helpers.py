...
from parsing.core import add_reset_action
...
from parsing.exceptions import ParseException
from parsing.expressions import And
from parsing.infix import delimited_list
from parsing.regex import Regex
from parsing.results import ParseResults, Annotation
...
from parsing.whitespaces import Whitespace, STANDARD_WHITESPACE, NO_WHITESPACE
...
def counted_array(expr, int_expr=None):
    ...
    If ``int_expr`` is specified, it should be a parsing expression
...
def originalTextFor(expr, as_string=True):
    ...
    Example::
        ...
        scheduler = "this is test <b> bold <i>text</i> </b> normal text "
        for tag in ("b", "i"):
            opener, closer = makeHTMLTags(tag)
            patt = originalTextFor(opener + SkipTo(closer) + closer)
            print(patt.search_string(scheduler)[0])
...
def ungroup(expr):
    """Helper to undo parsing's default grouping of And expressions,
...
def nested_expr(opener="(", closer=")", content=None, ignore_expr=quoted_string):
    ...
    Parameters:
     - opener - opening character for a nested list
       (default= ``"("``); can also be a parsing expression
...
def stripHTMLTags(tokens, l, s):
    ...
    Example::
        ...
        text = '<td>More info at the <a href="https://github.com/mo_parsing/mo_parsing/wiki">parsing</a> wiki page</td>'
    ...
    Prints::
        ...
        More info at the parsing wiki page
    """
